...
fn spawn_checks(
    ...
    for _ in 0..worker_count {
        ...
        thread::spawn(move || loop {
            // Recover from a poisoned mutex: a panic in one worker must not
            // cascade into every other worker panicking on `lock()`. The queue
            // is a plain `VecDeque`, so its contents stay consistent regardless.
            let job = queue
                .lock()
